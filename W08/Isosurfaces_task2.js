...

{
...

    {
        ...
        color_map.push( [ S, '0x' + color.getHexString() ] );
    ...
    material.color = new THREE.Color().setHex( color_map[ isovalue ][1] );
...

    {
        ...
        var v0_index = v0.x + (v0.y * volume.resolution.x) + (v0.z * (volume.resolution.x * volume.resolution.y));

        var v1_index = v1.x + (v1.y * volume.resolution.x) + (v1.z * (volume.resolution.x * volume.resolution.y));

        

        var s0 = volume.values[v0_index][0];

        var s1 = volume.values[v1_index][0];        
